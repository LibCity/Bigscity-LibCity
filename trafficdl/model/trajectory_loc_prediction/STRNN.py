...
class STRNN(AbstractModel):
    def __init__(self, config, data_feature):
        ...
        self.weight_ih = nn.Parameter(torch.randn(size=[self.hidden_size, self.hidden_size])) # C
        self.weight_th_upper = nn.Parameter(torch.randn(size=[self.hidden_size, self.hidden_size])) # T Tu
        self.weight_th_lower = nn.Parameter(torch.randn(size=[self.hidden_size, self.hidden_size])) # T Tl
        self.weight_sh_upper = nn.Parameter(torch.randn(size=[self.hidden_size, self.hidden_size])) # S
        self.weight_sh_lower = nn.Parameter(torch.randn(size=[self.hidden_size, self.hidden_size])) # S
    ...
    def forward(self, td_upper, td_lower, ld_upper, ld_lower, current_loc, loc_len):
        ...
        for i in range(batch_size):
            Ttd = [((self.weight_th_upper*td_upper[i][j] + self.weight_th_lower*td_lower[i][j])\
                    /(td_upper[i][j]+td_lower[i][j])) for j in range(loc_len[i])]
            Sld = [((self.weight_sh_upper*ld_upper[i][j] + self.weight_sh_lower*ld_lower[i][j])\
                    /(ld_upper[i][j]+ld_lower[i][j])) for j in range(loc_len[i])]
            loc = current_loc[i][:loc_len[i]] # sequence_len
            loc = self.location_weight(loc).unsqueeze(2)
            loc_vec = torch.sum(torch.cat([torch.mm(Sld[j], torch.mm(Ttd[j], loc[j]))\
                    .unsqueeze(0) for j in range(loc_len[i])], dim=0), dim=0) # 这里还差一个 Ch
            usr_vec = torch.mm(self.weight_ih, self.h0) # 这个就是 C*h
            hx = (loc_vec + usr_vec).reshape(1, self.hidden_size) # hidden_size x 1
            output.append(hx)
    ...
    def calculate_loss(self, batch):
        ...
        h_tq = self.forward(td_upper, td_lower, ld_upper, ld_lower, batch['current_loc'], loc_len) # batch_size * hidden_size
        dst = batch['target']
    ...
    def predict(self, batch):
        ...
        ret = torch.mm(user_vector, self.location_weight.weight.T) # batch_size * loc_size
        return ret
