...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, adj_mx):
        ...
        self.seq_len = int(config.get('seq_len'))  # for the encoder
...
class DCRNN(AbstractModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        # nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, config, adj_mx)
        self.encoder_model = EncoderModel(config, adj_mx)
        self.decoder_model = DecoderModel(config, adj_mx)
        self.cl_decay_steps = int(config.get('cl_decay_steps', 1000))
        self.use_curriculum_learning = bool(config.get('use_curriculum_learning', False))
        self.horizon = int(config.get('horizon', 1))
        self.seq_len = int(config.get('seq_len'))
        self.num_nodes = int(config.get('num_nodes', 1))
        self.input_dim = int(config.get('input_dim', 1))
        self.output_dim = int(config.get('output_dim', 1))
    ...
    def forward(self, batch, batches_seen=None):
        ...
        inputs = inputs.view(self.seq_len, batch_size, self.num_nodes * self.input_dim).to(device)
        self._logger.debug("X: {}".format(inputs.size()))

        if labels is not None:
            labels = labels.permute(1, 0, 2, 3)
            labels = labels[..., :self.output_dim].view(self.horizon, batch_size, self.num_nodes * self.output_dim).to(device)
        ...
        outputs = outputs.view(self.horizon, batch_size, self.num_nodes, self.output_dim).permute(1, 0, 2, 3)
        return outputs
