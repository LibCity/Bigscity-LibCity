...
class TrafficStateGridDataset(AbstractDataset):
    # TODO: grid也可以用三维张量
    ...
    def _generate_input_data(self, df):
        """
        根据全局参数`input_window`和`output_window`切分输入，产生模型需要的5维张量输入
        模型使用过去`input_window`长度的时间序列去预测未来`output_window`长度的时间序列
        :param df: ndarray (len_time, len_row, len_column, feature_dim)
        :return:
        # x: (epoch_size, input_length, len_row, len_column, feature_dim)
        # y: (epoch_size, output_length, len_row, len_column, feature_dim)
        """
        num_samples, len_row, len_column, feature_dim = df.shape
    ...
    def _generate_train_val_test(self):
        ...
        :return: x_train, y_train, x_val, y_val, x_test, y_test:
                    (num_samples, input_length, len_row, len_column, feature_dim)
        """
        df = self._load_grid_4d()  # (len_time, len_row, len_column, feature_dim)
        df = self._add_time_meta_information(df)
        # x: (num_samples, input_length, len_row, len_column, input_dim)
        # y: (num_samples, output_length, len_row, len_column, output_dim)
    ...
    def _load_cache_train_val_test(self):
        """
        加载之前缓存好的训练集、测试集、验证集
        :return: x_train, y_train, x_val, y_val, x_test, y_test: (num_samples, input_length, len_row, len_column, feature_dim)
        """
    ...
    def get_data(self):
        ...
        # x_train/y_train: (num_samples, input_length, len_row, len_column, feature_dim)
