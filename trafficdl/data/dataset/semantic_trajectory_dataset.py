...
class SemanticTrajectoryDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.feature_name = {'history_loc': 'int', 'history_tim': 'int', 'current_loc': 'int', 'current_tim': 'int', 'target': 'int', 'target_tim': 'int', 'uid': 'int', 'text': 'float'}
        ...
        for uid in user_set:
            ...
            for index, row in usr_traj.iterrows():
                if index == 0:
                    ...
                    # 处理第一个点的语义信息
                    useful_words_list = []
                    if self.config['dataset'] == 'foursquare_tky' or 'foursquare_nyk':
                        # TODO: 这种硬编码可能不太好
                        words = poi.iloc[row['location']]['venue_category_name'].split(' ')
                        for w in words:
                            w = w.lower()
            ...
            for i in range(sessions_len):
                ...
                for words in current_word_list:
                    ...
                    if len(words_index) == 0:
                        ...
                        words_vec = np.sum(word_one_hot_matrix[words_index], axis=0) / len(words_index)
