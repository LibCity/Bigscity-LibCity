...
def generate_dataloader(train_data, eval_data, test_data, batch_size, num_workers, pad_item=None, pad_max_len=None, shuffle=True):
    '''
    Args:
        train_data, eval_data, test_data (list): data 中每个元素是模型单次的输入
        batch_size (int)
        num_workers (int)
    ...
    def collator(indices):
        batch = Batch(pad_item, pad_max_len)
        for item in batch:
            batch.append(item)
        batch.padding()
        return batch
