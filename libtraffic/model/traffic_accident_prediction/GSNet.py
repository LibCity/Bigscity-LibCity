...
class GSNet(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.grid_node_map = data_feature.get('grid_node_map', torch.Tensor(size=(self.north_south_map, self.west_east_map)))
        ...
        self.risk_weights = data_feature.get('risk_thresholds', [])
    ...
    def forward(self, batch):
        ...
        graph_input = grid_input[:, :, self.graph_input_indices, :, :] \
                .reshape(batch_size, self.seq_len, len(self.graph_input_indices), self.west_east_map*self.north_south_map) \
    ...
    def calculate_loss(self, batch):
        ...
        for i in range(len(self.risk_thresholds)):
            if i == 0:
                weight[y_pred < self.risk_thresholds[i]] = self.risk_weights[i]
            else:
                weight[(y_pred > self.risk_thresholds[i-1]) & (y_pred <= self.risk_thresholds[i])] = self.risk_weights[i]
