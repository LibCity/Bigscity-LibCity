...
class IVMM(AbstractMapMatchingModel):
    ...
    def run(self, data):
        ...
        for key, value in trajectory.items():
            self._logger.info('begin to map trajectory with id:%d' % key)
            self.tra_id = key
            self.trajectory = value
            self._run_one_tra()
            self._logger.info('finish mapping trajectory with id:%d' % key)

        # res generated by res_dct
        res = []
        for key, lst in self.res_dct.items():
            tra_lst = trajectory[key]
            for i in range(len(lst)):
                res.append([tra_lst[i][0], lst[i]])
        return res
    ...
    def _find_matched_sequence(self):
        ...
        self.res_dct[self.tra_id] = list(map(lambda x: self.rd_nwk.edges[x]['rel_id'] if x is not None else None, res))