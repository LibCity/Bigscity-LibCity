...
class STEmbedding(nn.Module):
    ...
    def __init__(self, input_dim, D, bn_decay, device):
        self.device = device
...
class SSTBAN(AbstractTrafficStateModel):
    ...
    def __init__(self, config, data_feature):
        ...
        self.time_intervals = self.data_feature.get('time_intervals', 300)

        # config
        self.input_window = config.get('input_window', 24)
        self.output_window = config.get('output_window', 24)
        self.L = config.get('L', 2)
        self.K = config.get('K', 16)
        self.d = config.get('d', 8)
        self.node_miss_rate = config.get('node_miss_rate', 0.1)
        self.T_miss_len = config.get('T_miss_len', 12)
        self.reference_dim = config.get('reference_dim', 3)
        self.input_dim = int(24 * 60 * 60 / self.time_intervals) + 7
        self.bn_decay = config.get('bn_decay', 0.1)
        self.self_weight = config.get('self_weight', 0.05)
        D = self.K * self.d

        self.SE = nn.Parameter(torch.FloatTensor(self.num_nodes, D))
        self.STEmbedding = STEmbedding(self.input_dim, D, self.bn_decay, self.device)
        self.STAttBlock_1 = nn.ModuleList(
            [STAttBlock(self.K, self.d, self.input_window, self.num_nodes, self.reference_dim, self.bn_decay) for _ in
             range(self.L)])
        self.STAttBlock_self = nn.ModuleList(
            [STAttBlock_self(self.K, self.d, self.input_window, self.num_nodes, self.reference_dim, self.bn_decay) for
             _ in range(1)])
        self.STAttBlock_2 = nn.ModuleList(
            [STAttBlock(self.K, self.d, self.num_pred, self.num_nodes, self.reference_dim, self.bn_decay) for _ in
             range(self.L)])
        self.transformAttention = transformAttention(self.K, self.d, self.bn_decay)

        self.FC_1 = FC(input_dims=[self.feature_dim, D], units=[D, D], activations=[F.relu, None],
                       bn_decay=self.bn_decay)
        self.FC_2 = FC(input_dims=[D, D], units=[D, self.output_dim], activations=[F.relu, None],
                       bn_decay=self.bn_decay)
    ...
    def calculate_loss(self, batch):
        y_true = batch['y']
        y_predicted, complete_X_enc, X_miss = self.predict(batch)
        y_true = self._scaler.inverse_transform(y_true[..., :self.output_dim])
        y_predicted = self._scaler.inverse_transform(y_predicted[..., :self.output_dim])
        loss_batch = loss.masked_mae_torch(y_predicted, y_true, 0)
        loss_self = loss.masked_mse_torch(complete_X_enc, X_miss, 0)
